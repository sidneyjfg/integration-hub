...
export async function notifyGoogleChat(
  ...
  try {
    ...
    if (!webhookUrl) {
      console.error(
        "Erro: Variável de ambiente GOOGLE_CHAT_WEBHOOK_URL não definida.",
      );
      return;
    }
    ...
    const body =
      typeof message === "string" ? { text: message } : { cards: [message] }; // Se message for um objeto, envia como card
