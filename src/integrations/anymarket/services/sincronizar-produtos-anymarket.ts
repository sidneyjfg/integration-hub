...
export async function sincronizarProdutosAnymarket(): Promise<void> {
  ...
  try {
    ...
    while (hasMore) {
      console.log(
        `[Anymarket Produtos] Buscando produtos - offset: ${offset}, limit: ${limit}`,
      );
      ...
      if (response.content && response.content.length > 0) {
        ...
        const skusNesteBatch = produtos.reduce(
          (acc, p) => acc + (p.skus?.length || 0),
          0,
        );
    ...
    await notifyGoogleChatError(mensagemErro);
