...
export = async function runSincronizarPedidosNaoIntegradosTest(): Promise<void> {
  ...
  if (!address || typeof address === 'string') {
    throw new Error('Não foi possível iniciar o servidor local do Google Chat.')
  }
  ...
  try {
    ...
    assert.match(
      ...
      /Pedidos Pluggto não integrados encontrados/
    )
    assert.match(notifications[0].body.text, /Total: 2/)
    assert.match(
      notifications[0].body.text,
      /Filtro PLUGGTO_NO_LOOK_STATUS_TYPES: canceled/
    )
    assert.match(notifications[0].body.text, /Pedidos ignorados pelo filtro: 1/)
    assert.match(notifications[0].body.text, /PLG-1003/)
    assert.match(notifications[0].body.text, /PLG-1004/)
    assert.doesNotMatch(notifications[0].body.text, /PLG-1002/)
  } finally {
    await new Promise<void>((resolve, reject) => {
      server.close(error => {
        if (error) reject(error)
